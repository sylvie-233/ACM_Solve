...
class PrefixSum2D {
 ...
 public:
  ...
  void add(int x, int y, ll v) { a[x][y] += v; }
...
struct Diff2D {
  ...
  std::vector<std::vector<int>> build() {
    ...
    for (int i = 1; i <= n; ++i)
      for (int j = 1; j <= m; ++j)
        res[i][j] =
            diff[i][j] + res[i - 1][j] + res[i][j - 1] - res[i - 1][j - 1];
    return res;
  }
};
}  // namespace Diff
...
struct SparseTable {
  ...
  //   可自定义的区间合并函数（默认 max）
  static int mergeFunc(int a, int b) { return std::max(a, b); }
  ...
  void build(const std::vector<int>& arr) {
    ...
    for (int k = 1; k < K; ++k) {
      for (int i = 1; i + (1 << k) - 1 <= n; ++i) {
        st[k][i] = mergeFunc(st[k - 1][i], st[k - 1][i + (1 << (k - 1))]);
      }
    }
  }
  ...
  int query(int l, int r) const {
    int k = log2v[r - l + 1];
    return mergeFunc(st[k][l], st[k][r - (1 << k) + 1]);
  }
};
...
class SegTree {
 private:
  ...
  // PushUp: 更新父节点
  inline void pushup(int p) { tr[p].sum = tr[p << 1].sum + tr[p << 1 | 1].sum; }
 ...
 public:
  // 仅指定大小，默认全 0
  SegTree(int n) : n(n) { tr.assign(4 * n + 5, Node()); }
...
class StringHash1D {
 ...
 public:
  ...
  // 查询区间 [l, r]（1-index）
  ull query(int l, int r) const { return h[r] - h[l - 1] * p[r - l + 1]; }

  // 合并两个子串的哈希 (A followed by B)
  ull merge(ull h1, int len2, ull h2) const { return h1 * p[len2] + h2; }
};
...
class MatrixHash2D {
 ...
 public:
  // 构造：传入矩阵与两种底数
  MatrixHash2D(const std::vector<std::vector<int>>& a, ull base1 = 131,
               ull base2 = 233)
  ...
  void init(const std::vector<std::vector<int>>& a) {
    ...
    for (int i = 1; i <= n; ++i)
      for (int j = 1; j <= m; ++j) h[i][j] = h[i - 1][j] * base2 + h[i][j];
  }
