...
题目：给定一个长度为n的整数序列，请找出长度不超过m的连续子序列的最大和

思路：先把数组转为前缀和数组，再用`s[i]-min(s[j], j属于[i-m,i-1])`求最大和，利用滑动窗口求`min(s[j])`
...
最多进行k比交易
`f[i][j][0]`表示第i天买卖了j次手中无票的最大利润
`f[i][j][1]`表示第i天买卖了j次 手中有票的最大利润


##### 股票买卖含冰冻期

卖出股票后，无法在第二天买入（冰冻期为1天）
`f[i][0]`表示第i天手中无票的第1天最大利润
`f[i][1]`表示第i天手中有票的最大利润
`f[i][2]`表示第i天手中无票的第2天及以后最大利润
...
同一组内元素只能选一次（本质上同一组内是01背包）
`f[i][j]`表示前i组物品，能放入容量为j的背包的最大价值
...
有 n 堆石子，每次只能合并相邻两堆，合并代价 = 两堆石子之和
问：最小总代价是多少？

核心思路：枚举最后一次合并的断点 k（时间复杂度：O(n^3)）
`dp[l][r] = min(dp[l][k] + dp[k+1][r] + sum(l,r))`


环形石子合并，只需要把数组复制一边拆成链就行了
...
`dp[i][j]`表示已经种植了前i行，并且第i行状态为j的方案数
...
有依赖的背包
`f[u][j]`表示选择以u为子树的物品，在体积和不超过容量j时获得的最大价值
